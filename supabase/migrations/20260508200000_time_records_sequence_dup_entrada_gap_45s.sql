-- Duas «entradas» seguidas com 1–5 min (ou ~1 min) não passavam na tolerância >300s → E→E e o INSERT falhava.
-- Limiar: >45s entre E consecutivos no espelho (ou mesmo instante com rid maior).
...
BEGIN
  ...
  -- Tolerância: entrada após outra entrada no mesmo dia → NEW vira saída se > 45s
  IF v_t = 'entrada' THEN
    SELECT
    ...
    BEGIN
      ...
      LOOP
        ...
        WHERE tr.id::text = o.row_pk
          ...
          AND (
            EXTRACT(EPOCH FROM (o.inst - o.prev_inst)) > 45
...
COMMENT ON FUNCTION public.time_records_enforce_punch_sequence() IS
  'BEFORE INSERT: dupla entrada >45s ou mesmo instante (rid); sanduíche E–E–E ≤300s; reconciliação com bypass Portaria 671.';
