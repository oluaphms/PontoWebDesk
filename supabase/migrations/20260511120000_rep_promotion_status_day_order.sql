-- promotion_status: visibilidade operacional REP → espelho (sem relaxar o trigger).
-- Ordenação da promoção: dia civil (America/Sao_Paulo) + colaborador + data_hora (day-aware).

ALTER TABLE public.rep_punch_logs
  ADD COLUMN IF NOT EXISTS promotion_status text;

COMMENT ON COLUMN public.rep_punch_logs.promotion_status IS
  'promoted | promoted_partial | pending_sequence_resolution | rejected — derivado da última tentativa de promoção ao espelho.';

ALTER TABLE public.rep_punch_logs DROP CONSTRAINT IF EXISTS rep_punch_logs_promotion_status_check;
ALTER TABLE public.rep_punch_logs
  ADD CONSTRAINT rep_punch_logs_promotion_status_check CHECK (
    promotion_status IS NULL
    OR promotion_status IN (
      'promoted',
      'promoted_partial',
      'pending_sequence_resolution',
      'rejected'
    )
  );

CREATE INDEX IF NOT EXISTS idx_rep_punch_logs_company_promotion_status
  ON public.rep_punch_logs (company_id, promotion_status)
  WHERE promotion_status IS NOT NULL;

-- Backfill conservador
UPDATE public.rep_punch_logs r
SET promotion_status = CASE
  WHEN r.time_record_id IS NOT NULL THEN 'promoted'
  WHEN r.promotion_error_code = 'invalid_sequence' THEN 'pending_sequence_resolution'
  WHEN r.promotion_error_code IS NOT NULL THEN 'rejected'
  ELSE NULL
END
WHERE r.promotion_status IS NULL;

UPDATE public.rep_punch_logs r
SET promotion_status = 'promoted_partial'
WHERE r.time_record_id IS NOT NULL
  ...
  AND EXISTS (
    ...
    WHERE x.company_id = r.company_id
      ...
      AND btrim(x.resolved_user_id::text) = btrim(r.resolved_user_id::text)
...
BEGIN
  ...
  -- Alias `rp` (não `r`): após o FOR r IN ... LOOP, `r` continua no escopo PL/pgSQL e colide com o alias da tabela.
  UPDATE public.rep_punch_logs rp
  SET promotion_status = 'promoted_partial'
  WHERE btrim(rp.company_id::text) = v_cid
    ...
    AND EXISTS (
      SELECT 1
      FROM public.rep_punch_logs x
      WHERE btrim(x.company_id::text) = v_cid
        AND COALESCE(x.ignored, false) = false
        AND x.time_record_id IS NULL
        AND x.resolved_user_id IS NOT NULL
...
COMMENT ON FUNCTION public.rep_promote_pending_rep_punch_logs(text, uuid, timestamptz, timestamptz, uuid, uuid) IS
  'Promove rep_punch_logs; ordenação day-aware; promotion_status; falha por linha não aborta o lote.';
