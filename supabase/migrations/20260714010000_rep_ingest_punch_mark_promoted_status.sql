-- Consistência pós-promoção inline em rep_ingest_punch:
-- quando time_record é criado com sucesso, align status/promotion_status com
-- rep_promote_pending_rep_punch_logs (sem alterar lógica de insert/espelho/671).
-- Nota: não existe coluna promoted_at; last_promotion_attempt_at registra o momento.

CREATE OR REPLACE FUNCTION public.rep_ingest_punch(p_company_id text, p_rep_device_id uuid DEFAULT NULL::uuid, p_pis text DEFAULT NULL::text, p_cpf text DEFAULT NULL::text, p_matricula text DEFAULT NULL::text, p_nome_funcionario text DEFAULT NULL::text, p_data_hora timestamp with time zone DEFAULT NULL::timestamp with time zone, p_tipo_marcacao text DEFAULT NULL::text, p_nsr bigint DEFAULT NULL::bigint, p_raw_data jsonb DEFAULT '{}'::jsonb, p_only_staging boolean DEFAULT false, p_apply_schedule boolean DEFAULT false, p_force_user_id uuid DEFAULT NULL::uuid, p_trust_client_identity boolean DEFAULT false, p_punch_hash text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Reparo de linhas já promovidas (TR existente) mas com status pendente
UPDATE public.rep_punch_logs
SET
  status = 'promoted',
  promotion_status = 'promoted',
  promotion_error_code = NULL,
  promotion_error_message = NULL,
  last_promotion_attempt_at = COALESCE(last_promotion_attempt_at, NOW())
WHERE time_record_id IS NOT NULL
  AND COALESCE(ignored, false) = false
  AND (
    status IS DISTINCT FROM 'promoted'
    OR promotion_status IS DISTINCT FROM 'promoted'
  );
