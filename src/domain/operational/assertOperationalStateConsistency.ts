...
import { punchInstantOperationalYmd } from '../../services/monitoring/monitoringGeoHardLock.service';
...
export function assertOperationalStateConsistency(params: {
  ...
  const { companyId, usingCos, cosByEmployee, pipelineRows, todayYmd } = params;
  if (!usingCos) {
  ...
  for (const row of pipelineRows) {
    ...
    if (!cos) continue;
    if (todayYmd && cos.last_punch_at) {
      const cosDay = punchInstantOperationalYmd({
        timestamp: cos.last_punch_at,
        created_at: cos.last_punch_at,
      });
      if (cosDay !== todayYmd) continue;
    }
