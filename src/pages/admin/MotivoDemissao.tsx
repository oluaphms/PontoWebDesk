...
const AdminMotivoDemissao: React.FC = () => {
  ...
  const load = async () => {
    ...
    setLoadingData(true);
    try {
      const data = (await db.select('motivo_demissao', [{ column: 'company_id', operator: 'eq', value: user.companyId }])) as any[];
  ...
  const handleSave = async (e?: React.MouseEvent) => {
    ...
    if (!trimmed) {
      setModalError('Informe a descrição do motivo de demissão.');
      return;
    }
    setSaving(true);
    setModalError(null);
    ...
    try {
      if (editingId) {
        await db.update('motivo_demissao', editingId, { name: trimmed });
        setMessage({ type: 'success', text: 'Motivo de demissão atualizado com sucesso.' });
      } else {
        await db.insert('motivo_demissao', {
          id: crypto.randomUUID(),
          company_id: user.companyId,
          name: trimmed,
          created_at: new Date().toISOString(),
        });
        setMessage({ type: 'success', text: 'Motivo de demissão cadastrado com sucesso.' });
      }
      setModalOpen(false);
      load();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Excluir este motivo de demissão? Funcionários com este motivo manterão o dado no histórico.')) return;
    try {
      await db.delete('motivo_demissao', id);
      setMessage({ type: 'success', text: 'Motivo de demissão excluído.' });
      load();
    } catch (e: any) {
    ...
    <RoleGuard user={user} allowedRoles={['admin', 'hr']}>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <PageHeader
            title="Motivos de Demissão"
            subtitle="Cadastro de motivo de demissão do funcionário. Utilizado no cadastro de funcionários."
            icon={<UserX size={24} />}
          />
          <button
            type="button"
          ...
          {loadingData ? (
            ...
            <>
                ...
                <tbody>
                  {rows.map((row, idx) => (
                    <tr key={row.id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-400 font-mono text-xs">{row.id.slice(0, 8)}</td>
              ...
              {!loadingData && rows.length === 0 && (
                <p className="p-8 text-center text-slate-500 dark:text-slate-400">Nenhum motivo de demissão cadastrado. Clique em &quot;Incluir&quot; para começar.</p>
              )}
            </>
          )}
        </div>
        ...
        {modalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" role="dialog" aria-modal="true" onClick={() => !saving && setModalOpen(false)}>
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 w-full max-w-md p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">{editingId ? 'Editar motivo de demissão' : 'Novo motivo de demissão'}</h3>
              {modalError && (
                <div className="p-3 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 text-sm">
                  {modalError}
                </div>
              )}
              <p className="text-sm text-slate-500 dark:text-slate-400">Código: gerado automaticamente pelo sistema.</p>
              <div>
                ...
                <input
                  ...
                  placeholder="Descrição do motivo de demissão do funcionário"
                />
              </div>
...
export default AdminMotivoDemissao;
