/**
 * LicenseService — fachada pública de licenciamento (frontend).
 *
...
import { LicenseCache } from './license/licenseCache';
import { LicenseResolver } from './license/licenseResolver';
...
export const LicenseService = {
  ...
  /** Sem chave/tier/payload = full (compatibilidade com o produto atual). */
  getTier(): LicenseTier {
  ...
  /** Ativa = licenciada e não vencida (somente relatório nesta fase). */
  isActive(): boolean {
  ...
  /**
   * Entitlement por payload/tier — não depende de isActive nesta fase
   * (vencimento report-only).
   */
