...
export interface NavigationItemSchema {
  ...
  nameKey: string;
  /** Subtítulo opcional no menu (ex: Smart Dock) */
  hintKey?: string;
  path: string;
...
export const navigationGroups: Record<string, NavigationGroupSchema> = {
  ...
  time: {
    ...
    items: [
      ...
      { name: 'Cálculos', nameKey: 'menu.calculos', path: '/admin/calculos', roles: ['admin', 'hr'] },
      ...
      { name: 'Dashboard RH BH', nameKey: 'menu.bankHoursDashboard', path: '/admin/bank-hours/dashboard', roles: ['admin', 'hr'] },
