...
async function authenticateDevice(
  ...
  if (!token) return { ok: false, response: noCache(Response.json({ error: 'Unauthorized' }, { status: 401 })) };
  ...
  if (deviceErr || !requestedDevice) {
    return { ok: false, response: noCache(Response.json({ error: 'Dispositivo não encontrado' }, { status: 404 })) };
  }
...
async function handlePendingUsers(request: Request, supabase: AnySupabase, deviceId: string, headers: Record<string, string>) {
  if (request.method !== 'GET') return noCache(Response.json({ error: 'Method not allowed' }, { status: 405, headers }));
  ...
  if (error) return noCache(Response.json({ error: 'Falha ao buscar pendências' }, { status: 500, headers }));
...
async function handleAckSync(request: Request, supabase: AnySupabase, deviceId: string, headers: Record<string, string>) {
  if (request.method !== 'POST') return noCache(Response.json({ error: 'Method not allowed' }, { status: 405, headers }));
  const auth = await authenticateDevice(supabase, request, deviceId);
  if (!auth.ok) {
    const res = authResponse(auth);
    return noCache(new Response(await res.text(), { status: res.status, headers: { ...headers, 'Content-Type': 'application/json' } }));
  ...
  try { body = await request.json(); } catch { return noCache(Response.json({ error: 'Body inválido' }, { status: 400, headers })); }
  ...
  if (!syncId) return noCache(Response.json({ error: 'sync_id é obrigatório' }, { status: 400, headers }));
  const { data: existing, error: existingErr } = await supabase
    .from('device_user_sync')
    .select('id, device_id, sync_attempts, sync_status, sync_error, external_id_on_device')
    .eq('id', syncId)
    .maybeSingle();
  if (existingErr || !existing) return noCache(Response.json({ error: 'sync_id não encontrado' }, { status: 404, headers }));
  if (existing.device_id !== deviceId) return noCache(Response.json({ error: 'sync_id não pertence ao dispositivo' }, { status: 403, headers }));
  if (success) {
    if (existing.sync_status === 'sent' && String(existing.external_id_on_device || '') === String(externalId || '')) {
      return noCache(Response.json({ success: true, idempotent: true }, { status: 200, headers }));
    }
    ...
    if (error) return noCache(Response.json({ error: 'Falha ao confirmar sincronização' }, { status: 500, headers }));
  ...
  if (sameErrorAlready) return noCache(Response.json({ success: true, idempotent: true }, { status: 200, headers }));
  ...
  if (error) return noCache(Response.json({ error: 'Falha ao registrar erro de sincronização' }, { status: 500, headers }));
  return noCache(Response.json({ success: true }, { status: 200, headers }));
}
...
async function handleSyncStatus(request: Request, supabase: AnySupabase, deviceId: string, headers: Record<string, string>) {
  ...
  cachePrivate(syncRes, 5);
  varyAuthorization(syncRes);
  return syncRes;
}
...
async function handleForceSync(request: Request, supabase: AnySupabase, deviceId: string, headers: Record<string, string>) {
  ...
  if (error) return noCache(Response.json({ error: 'Falha ao forçar reprocessamento' }, { status: 500, headers }));
  return noCache(Response.json({ success: true }, { status: 200, headers }));
}
...
export async function handleDeviceSyncRoute(request: Request, repSlug: string): Promise<Response> {
  ...
  try {
    ...
    return noCache(Response.json({ error: 'Supabase não configurado' }, { status: 500, headers: corsHeaders }));
  }
  ...
  if (parts.length < 3 || parts[0] !== 'devices') {
    return noCache(Response.json({ error: 'Rota inválida' }, { status: 404, headers: corsHeaders }));
  }
  const deviceId = decodeURIComponent(parts[1] || '');
  const action = decodeURIComponent(parts[2] || '');
  if (!deviceId) return noCache(Response.json({ error: 'device_id é obrigatório' }, { status: 400, headers: corsHeaders }));
  ...
  return noCache(Response.json({ error: 'Rota não encontrada' }, { status: 404, headers: corsHeaders }));
}
