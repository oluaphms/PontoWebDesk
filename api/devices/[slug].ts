...
import { getSecureCorsHeaders, extractBearerToken, secureCompare } from '../_shared/security.js';
...
function extractPathParts(request: Request): string[] {
...
async function authenticateDevice(
  ...
  request: Request,
  deviceId: string
): Promise<{ ok: true; device: DeviceRow } | { ok: false; response: Response }> {
  ...
  if (device.api_key && secureCompare(token, device.api_key)) {
...
async function authenticateAdminJwtForDevice(
  supabase: ReturnType<typeof createClient>,
  token: string,
  device: DeviceRow
): Promise<boolean> {
...
async function authenticateDeviceOrAdmin(
  ...
  if (isAdmin) return { ok: true, device: deviceData as DeviceRow };
  return baseAuth;
}
...
function normalizeSyncError(raw: unknown): StructuredSyncError {
  if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
    ...
    const code = SYNC_ERROR_CODES.has(codeCandidate) ? codeCandidate : 'UNKNOWN';
    return { code: code as StructuredSyncError['code'], message };
  }
  const message = String(raw || 'Erro não informado pelo agente').trim();
  return { code: 'UNKNOWN', message };
}

async function markOfflineDevices(supabase: ReturnType<typeof createClient>, companyId?: string): Promise<void> {
  let query = supabase
    .from('rep_devices')
...
async function handlePendingUsers(
  request: Request,
  supabase: ReturnType<typeof createClient>,
  deviceId: string,
  ...
  const { data: claimedRows, error: claimErr } = await supabase.rpc('claim_device_user_sync_batch', {
    p_device_id: deviceId,
    p_limit: 100,
  });

  if (claimErr) {
    console.error('[DEVICE_USER_SYNC][PENDING][ERROR]', claimErr);
    return Response.json({ error: 'Falha ao buscar pendências' }, { status: 500, headers: corsHeaders });
  }
  ...
  const claimed = Array.isArray(claimedRows)
    ? (claimedRows as Array<{
        ...
        user_id: string;
        identifier: string;
        identifier_type: 'cpf' | 'pis' | 'both';
        external_id_on_device: string | null;
      }>)
    : [];

  const userIds = claimed.map((row) => row.user_id);
  ...
  if (userIds.length > 0) {
    ...
    if (usersErr) {
      console.error('[DEVICE_USER_SYNC][PENDING][USERS_ERROR]', usersErr);
    } else if (Array.isArray(usersData)) {
      namesById = new Map(usersData.map((u) => [String(u.id), String(u.nome || 'Sem nome')]));
    }
  }
  ...
  return Response.json(
    {
      ...
      users: claimed.map((row) => ({
        sync_id: row.sync_id,
        user_id: row.user_id,
        name: namesById.get(row.user_id) || 'Sem nome',
        identifier: row.identifier,
        identifier_type: row.identifier_type,
        external_id_on_device: row.external_id_on_device || null,
      })),
    },
...
async function handleAckSync(
  ...
  let body: {
    sync_id?: string;
    success?: boolean;
    external_id_on_device?: string;
    error?: string | StructuredSyncError;
  };
  try {
  ...
  if (!syncId) {
    return Response.json({ error: 'sync_id é obrigatório' }, { status: 400, headers: corsHeaders });
  }
  ...
  if (existingErr || !existing) {
    return Response.json({ error: 'sync_id não encontrado' }, { status: 404, headers: corsHeaders });
  }

  if (existing.device_id !== deviceId) {
    return Response.json({ error: 'sync_id não pertence ao dispositivo' }, { status: 403, headers: corsHeaders });
  }

  if (success) {
    const alreadySent =
      existing.sync_status === 'sent' &&
      String(existing.external_id_on_device || '') === String(externalId || '');
    if (alreadySent) {
      return Response.json({ success: true, idempotent: true }, { status: 200, headers: corsHeaders });
    }
    const { error: updateErr } = await supabase
      ...
      .update({
        sync_status: 'sent',
        sync_error: null,
        external_id_on_device: externalId,
    ...
    if (updateErr) {
      console.error('[DEVICE_USER_SYNC][ACK][SUCCESS][ERROR]', updateErr);
      return Response.json({ error: 'Falha ao confirmar sincronização' }, { status: 500, headers: corsHeaders });
    }
  ...
  if (updateErr) {
    console.error('[DEVICE_USER_SYNC][ACK][ERROR]', updateErr);
    return Response.json({ error: 'Falha ao registrar erro de sincronização' }, { status: 500, headers: corsHeaders });
  }
...
async function handleHeartbeat(
  ...
  if (error) {
    return Response.json({ error: 'Falha ao registrar heartbeat' }, { status: 500, headers: corsHeaders });
  }
  return Response.json({ success: true }, { status: 200, headers: corsHeaders });
}
...
async function handleSyncStatus(
  ...
  if (error) {
    return Response.json({ error: 'Falha ao carregar métricas de sincronização' }, { status: 500, headers: corsHeaders });
  }
  ...
  const { data: deviceNow } = await supabase
    .from('rep_devices')
...
async function handleForceSync(
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      sync_attempts: 0,
      sync_error: null,
      external_id_on_device: null,
      updated_at: new Date().toISOString(),
    })
    .eq('device_id', deviceId)
    .in('sync_status', ['error', 'sent']);

  if (error) {
    return Response.json({ error: 'Falha ao forçar reprocessamento' }, { status: 500, headers: corsHeaders });
  }
...
async function handler(request: Request): Promise<Response> {
  ...
  const parts = extractPathParts(request);
  if (parts.length < 4 || parts[0] !== 'api' || parts[1] !== 'devices') {
  ...
  if (!deviceId) {
    return Response.json({ error: 'device_id é obrigatório' }, { status: 400, headers: corsHeaders });
  }

  if (action === 'pending-users') {
    return handlePendingUsers(request, supabase, deviceId, corsHeaders);
  }

  if (action === 'ack-sync') {
    return handleAckSync(request, supabase, deviceId, corsHeaders);
  }

  if (action === 'heartbeat') {
    return handleHeartbeat(request, supabase, deviceId, corsHeaders);
  }

  if (action === 'sync-status') {
    return handleSyncStatus(request, supabase, deviceId, corsHeaders);
  }

  if (action === 'force-sync') {
    return handleForceSync(request, supabase, deviceId, corsHeaders);
  }
