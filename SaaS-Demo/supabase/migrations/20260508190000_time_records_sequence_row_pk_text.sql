-- Correcção: time_records.id no projeto é TEXT (UUID em string) em vários fluxos (rep_promote).
-- day_rows com NULL::uuid + coluna id mista impedia UNION / JOIN fiável no UPDATE de reconciliação.
-- Usa row_pk text (tr.id::text) em todo o CTE e WHERE tr.id::text = o.row_pk.
...
COMMENT ON FUNCTION public.prevent_update_delete_time_records() IS
  'Bloqueia UPDATE/DELETE em batidas REP/app; permite manuais; permite ajuste automático de sequência (GUC ponto.time_record_sequence_reconcile + sequence_adjusted em raw_data).';
...
COMMENT ON FUNCTION public.time_records_enforce_punch_sequence() IS
  'BEFORE INSERT: valida E/P/S; reconcilia com row_pk::text e bypass Portaria 671 (GUC + sequence_adjusted).';
