-- Match na consolidação: usar matricula_derived do JSON Control iD (raw_data) quando a coluna matricula está vazia.
-- Corrige casos em que o PIS no AFD vem truncado mas o relógio já enviou a matrícula (access_log) no payload.
...
COMMENT ON FUNCTION public.rep_promote_pending_rep_punch_logs(text, uuid, timestamptz, timestamptz, uuid) IS
  'Promove rep_punch_logs; PIS efectivo em raw_data; matricula_derived no JSON quando coluna matricula vazia.';
