-- Consolidação da fila rep_punch_logs: janela opcional (mesmo critério que «só hoje» no cliente)
-- e filtro opcional por utilizador (só promove se o colaborador resolvido for o indicado).

DROP FUNCTION IF EXISTS public.rep_promote_pending_rep_punch_logs(text, uuid);
...
COMMENT ON FUNCTION public.rep_promote_pending_rep_punch_logs(text, uuid, timestamptz, timestamptz, uuid) IS
  'Promove rep_punch_logs para time_records. Janela local opcional (início/fim timestamptz); p_only_user_id restringe a quem casa no cadastro.';
