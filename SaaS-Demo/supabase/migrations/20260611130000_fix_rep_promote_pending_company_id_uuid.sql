-- Corrige rep_promote_pending_rep_punch_logs após company_id UUID em time_records.
-- Alinhado a 20260520350000_fix_rep_ingest_punch_uuid_text.sql (v_company_uuid).
...
COMMENT ON FUNCTION public.rep_promote_pending_rep_punch_logs(text, uuid, timestamptz, timestamptz, uuid, uuid) IS
  'Promove rep_punch_logs; company_id UUID nativo em time_records; falha por linha não aborta o lote.';
...
-- fix_and_promote_pending_rep_punches: mesmo ajuste em lookup/INSERT time_records
...
COMMENT ON FUNCTION public.fix_and_promote_pending_rep_punches(uuid, int) IS
  'Reprocessa em lote com company_id UUID nativo em time_records.';
