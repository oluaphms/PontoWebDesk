-- Espelho: não promover batidas sem identidade resolvida na fila (rep_punch_logs.resolved_user_id).
...
BEGIN
  ...
  LOOP
    IF r.resolved_user_id IS NULL OR btrim(COALESCE(r.resolved_user_id, '')) = '' THEN
      RAISE WARNING '[REP SKIP] unresolved_identity nsr=%', r.nsr;
...
COMMENT ON FUNCTION public.rep_promote_pending_rep_punch_logs(text, uuid, timestamptz, timestamptz, uuid) IS
  'Promove rep_punch_logs só com resolved_user_id; sem identidade → SKIP [REP SKIP] unresolved_identity; timeout 600s.';
