-- Consolidação REP: quando o PIS no log é DV-válido mas errado, casar pelo prefixo do campo
-- identificador AFD (blob) com users.numero_identificador (ex.: 29667427651 → blob 296674276539…).
...
BEGIN
  ...
  LOOP
    ...
    IF r.raw_data IS NOT NULL AND jsonb_typeof(r.raw_data) = 'object' THEN
      v_raw_line := NULLIF(trim(r.raw_data->>'raw'), '');
      IF v_raw_line IS NULL AND jsonb_typeof(r.raw_data->'raw') = 'object' THEN
        v_raw_line := NULLIF(trim(r.raw_data->'raw'->>'raw'), '');
      END IF;
    END IF;
...
COMMENT ON FUNCTION public.rep_promote_pending_rep_punch_logs(text, uuid, timestamptz, timestamptz, uuid) IS
  'Promove rep_punch_logs; fallback prefixo numero_identificador vs blob AFD; timeout local; ignora ignored.';
