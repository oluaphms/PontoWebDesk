-- Idempotência forte: punch_hash (SHA-256) + índice semântico (device + PIS + data_hora).
-- Permite reenvio do agente sem duplicar; last-nsr permanece otimização local.
...
COMMENT ON COLUMN public.rep_punch_logs.punch_hash IS
  'SHA-256 hex: device_id|pis_canonical|data_hora_utc|nsr — chave idempotente de ingestão.';
...
COMMENT ON FUNCTION public.rep_compute_punch_hash(uuid, text, timestamptz, bigint) IS
  'Hash idempotente alinhado ao agente REP (device|pis|timestamp|nsr).';

UPDATE public.rep_punch_logs l
SET punch_hash = public.rep_compute_punch_hash(l.rep_device_id, COALESCE(l.pis, l.cpf), l.data_hora, l.nsr)
WHERE l.punch_hash IS NULL
  AND l.data_hora IS NOT NULL;

-- Deduplica hash antes do índice único (mantém linha com espelho ou mais recente)
DELETE FROM public.rep_punch_logs t
WHERE t.id IN (
  SELECT id FROM (
    SELECT id,
           ROW_NUMBER() OVER (
             PARTITION BY punch_hash
...
DROP INDEX IF EXISTS public.idx_rep_punch_hash;
CREATE UNIQUE INDEX idx_rep_punch_hash
  ON public.rep_punch_logs (punch_hash)
  WHERE punch_hash IS NOT NULL;

DROP INDEX IF EXISTS public.idx_rep_punch_semantic;
CREATE UNIQUE INDEX idx_rep_punch_semantic
  ON public.rep_punch_logs (
    company_id,
    dedupe_device,
    COALESCE(public.rep_afd_canonical_11_digits(pis), public.rep_afd_canonical_11_digits(cpf), ''),
    data_hora
  )
  WHERE data_hora IS NOT NULL
    AND COALESCE(public.rep_afd_canonical_11_digits(pis), public.rep_afd_canonical_11_digits(cpf), '') <> '';

COMMENT ON INDEX public.idx_rep_punch_semantic IS
  'Proteção semântica: mesmo relógio + identificador + instante (NSR diferente no firmware).';

CREATE TABLE IF NOT EXISTS public.rep_device_checkpoints (
  rep_device_id uuid PRIMARY KEY REFERENCES public.rep_devices(id) ON DELETE CASCADE,
  company_id uuid NOT NULL,
  last_nsr bigint,
  last_punch_hash text,
  updated_at timestamptz NOT NULL DEFAULT now()
);

COMMENT ON TABLE public.rep_device_checkpoints IS
  'Checkpoint opcional por relógio (último NSR/hash aceito no servidor).';

ALTER TABLE public.rep_device_checkpoints ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS rep_device_checkpoints_service ON public.rep_device_checkpoints;
CREATE POLICY rep_device_checkpoints_service ON public.rep_device_checkpoints
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- Pré-checagem idempotente (evita trabalho de match quando já ingerido)
