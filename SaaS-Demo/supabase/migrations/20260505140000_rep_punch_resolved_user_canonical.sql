-- rep_punch_logs: utilizador canónico resolvido + raw_data.canonical_user_id em ingest/promote.

ALTER TABLE public.rep_punch_logs
ADD COLUMN IF NOT EXISTS resolved_user_id TEXT;

COMMENT ON COLUMN public.rep_punch_logs.resolved_user_id IS
  'users.id (text) após match REP (PIS/CPF/crachá/blob ou últimos 8 únicos); espelha raw_data.canonical_user_id.';

-- ---------------------------------------------------------------------------
-- rep_ingest_punch: resolved_user_id + canonical_user_id no JSON
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_user_id IS NOT NULL THEN
    v_raw_out := COALESCE(v_raw_out, '{}'::jsonb) || jsonb_build_object('canonical_user_id', v_user_id);
  END IF;
...
) IS 'Ingere marcação REP; resolved_user_id + raw_data.canonical_user_id; tiered + blob; fallback.';
...
-- ---------------------------------------------------------------------------
-- rep_promote_pending_rep_punch_logs: gravar identidade canónica na fila
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.rep_promote_pending_rep_punch_logs(text, uuid, timestamptz, timestamptz, uuid) IS
  'Promove rep_punch_logs; resolved_user_id + raw_data.canonical_user_id; promoted_detail para espelho; timeout 600s.';
