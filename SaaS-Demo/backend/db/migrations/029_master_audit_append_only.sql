-- 029 — Auditoria Master append-only (Fase 5.1)
-- Bloqueia UPDATE / DELETE / TRUNCATE em public.master_audit.
-- Não altera colunas, API HTTP nem formato dos registros.
-- Idempotente.

BEGIN;

-- ---------------------------------------------------------------------------
-- 1) Função + triggers (bloqueiam mesmo o owner da tabela)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2) Privilégios: sem UPDATE / DELETE / TRUNCATE para papéis da aplicação
--    SELECT + INSERT permanecem (autorização de leitura continua na API
--    via audit:read — OWNER/AUDITOR e demais perfis já mapeados).
-- ---------------------------------------------------------------------------
