...
import { enrichExecutiveSummary, withExecutiveDefaults } from './executiveEnrichment.js';
...
describe('executiveEnrichment defaults', () => {
  ...
  it('usa pagamentos como fonte única dos recebimentos no fallback in_memory', async () => {
    ...
    expect(enriched.monthlyRevenueCents).toBe(12_000);
    expect(enriched.annualRevenueCents).toBe(12_000);
    expect(enriched.revenueCents).toBe(12_000);
    expect(enriched.revenue.monthReceiptsCents).toBe(12_000);
  });
});
