...
/** Garante PK `id` em INSERT genérico quando o cliente omite (ou manda null/vazio). */
async function ensureInsertRowId(
  table: string,
  row: Record<string, unknown>,
): Promise<void> {
  if (!(await tableHasColumn(table, 'id'))) return;
...
export async function insertDataController(req: AuthedRequest, res: Response): Promise<void> {
  ...
  try {
    ...
    await ensureInsertRowId(table, row);
