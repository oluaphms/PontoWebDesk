    ...
    <section className={repPageUi.c015} aria-labelledby="rep-hub-collect-diag">
      ...
      {loading && !snapshot ? (
        ...
        <ul className="text-sm space-y-1 text-slate-700 dark:text-slate-300">
          ...
          <li>
            <strong>No escopo (datas):</strong>{' '}
            {d.afd_in_scope ?? (result.parsed != null ? String(result.parsed) : '—')}
          </li>
          <li>
            <strong>Enfileiradas (agente):</strong>{' '}
            {d.queued ?? (result.sent_ok != null ? String(result.sent_ok) : '—')}
          </li>
          {(d.duplicates ?? 0) > 0 ? (
            <li>
