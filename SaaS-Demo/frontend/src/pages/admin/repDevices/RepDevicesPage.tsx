...
import { RepDetailReadModal } from './RepDetailReadModal';
import { RepDeviceDeleteModal } from './RepDeviceDeleteModal';
import { RepDeviceFormModal, type RepDeviceFormState } from './RepDeviceFormModal';
import { RepDeviceUsersModal } from './RepDeviceUsersModal';
import { RepPendingPisDiagnosticsModal } from './RepPendingPisDiagnosticsModal';
...
const AdminRepDevices: React.FC = () => {
  ...
  const [form, setForm] = useState<RepDeviceFormState>({
    nome_dispositivo: '',
    fabricante: '',
    modelo: '',
    ip: '',
    ...
    try {
      ...
      if (r.command_id) {
        ...
        if (poll.ok) {
          ...
          const failedPoll = poll as Extract<RepCommandPollOutcome, { ok: false }>;
          if (failedPoll.status === 'timeout') {
          ...
          setMessage({ type: 'warning', text: 'Coleta ainda em andamento ou agente demorou demais.' });
            ...
            appendSrLog(`Falha na coleta: ${failedPoll.message}`, 'error');
            setMessage({ type: 'error', text: failedPoll.message });
          }
        }
    ...
    async (id: string) => {
      ...
      try {
        ...
        if (outcome.ok) {
          ...
          const failedOutcome = outcome as Extract<PollTestConnectionOutcome, { ok: false }>;
          ...
          const timeoutText =
            failedOutcome.timedOut && device
              ? buildAgentCommandTimeoutMessage(device, true, lastSeen)
              : failedOutcome.message === 'AGENT_COMMAND_TIMEOUT' && device
                ? buildAgentCommandTimeoutMessage(device, true, lastSeen)
          ...
          if (failedOutcome.timedOut && device && !agentOnline) {
  ...
  const ignoreUnidentifiedPendingPunches = async (deviceOverride?: RepDeviceRow | null) => {
    ...
    try {
      const queueQuery = (client as unknown as {
        from: (table: string) => {
      ...
      const { data: rawRows, error } = await queueQuery
        .neq('ignored', true)
        .order('data_hora', { ascending: true })
        .limit(500);
      if (error) throw new Error(error.message);
      const queueRows = (rawRows ?? []) as Array<{
        id: string;
        nsr: number | null;
        pis: string | null;
        cpf: string | null;
      ...
      const toIgnore = filterActiveRepPunchLogs(queueRows).filter((row) => {
  ...
  const tryAutoRepairPendingMatches = async (
    ...
    type RepQueueRepairQuery = {
      gte: (column: string, value: string) => RepQueueRepairQuery;
      lte: (column: string, value: string) => RepQueueRepairQuery;
      limit: (n: number) => Promise<{ data: Array<Record<string, unknown>> | null; error: { message: string } | null }>;
    };
    let q = (client as unknown as {
      from: (table: string) => {
        select: (columns: string) => {
          eq: (column: string, value: string) => {
            eq: (column: string, value: string) => {
      ...
      .is('time_record_id', null)
      .neq('ignored', true) as RepQueueRepairQuery;
  ...
  const tryFallbackPromotePendingByLocalMatch = async (
    ...
    type RepQueuePromoteQuery = {
      gte: (column: string, value: string) => RepQueuePromoteQuery;
      lte: (column: string, value: string) => RepQueuePromoteQuery;
      order: (
        column: string,
        opts: { ascending: boolean },
      ) => { limit: (n: number) => Promise<{ data: Array<Record<string, unknown>> | null; error: { message: string } | null }> };
    ...
    let q = (client as unknown as {
      ...
      .eq('rep_device_id', deviceId)
      .is('time_record_id', null)
      .neq('ignored', true) as RepQueuePromoteQuery;
  ...
  return (
    <div className={cx('p-4 md:p-6 lg:p-10 max-w-7xl mx-auto w-full min-w-0 flex flex-col', uiTokens.spacing.sectionGap)}>
      ...
      <RepPendingPisDiagnosticsModal
        open={pendingPisModal.open}
        rows={pendingPisModal.rows}
        employees={employees}
        showIgnoredPunches={showIgnoredPunches}
        onShowIgnoredChange={(checked) => {
          setShowIgnoredPunches(checked);
          loadPendingPisDiagnostics();
        }}
        selectedEmployeeForReassign={selectedEmployeeForReassign}
        onSelectedEmployeeChange={setSelectedEmployeeForReassign}
        selectedPunches={selectedPunches}
        onSelectedPunchesChange={setSelectedPunches}
        reassigningPunches={reassigningPunches}
        ignoringPunches={ignoringPunches}
        onClose={() => setPendingPisModal({ open: false, rows: [] })}
        onRefresh={() => {
          loadPendingPisDiagnostics();
        }}
        onReassign={reassignPendingPunches}
        onIgnore={ignoreSelectedPunches}
        onGoToEmployees={() => {
          setPendingPisModal({ open: false, rows: [] });
          window.location.href = '/admin/employees';
        }}
        findEmployeeByPis={findEmployeeByPis}
      />
      ...
      <RepDeviceFormModal
        ...
        form={form}
        setForm={setForm}
        configExtraBaseline={configExtraBaseline}
