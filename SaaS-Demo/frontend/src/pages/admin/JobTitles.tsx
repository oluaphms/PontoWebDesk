...
import { fetchCachedJobTitles } from '../../services/catalogCache.service';
...
const AdminJobTitles: React.FC = () => {
  ...
  const load = async () => {
    ...
    setLoadingData(true);
    try {
      const data = await fetchCachedJobTitles(user.companyId);
      setRows((data ?? []).map((r) => ({
        id: String(r.id ?? ''),
        name: String(r.name || ''),
  ...
  const handleSave = async (e?: React.MouseEvent) => {
    ...
    if (!trimmed) {
      setModalError('Informe o nome do cargo.');
      return;
    }
    setSaving(true);
    setModalError(null);
