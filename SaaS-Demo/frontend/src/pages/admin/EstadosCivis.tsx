...
import { fetchCachedEstadosCivis } from '../../services/catalogCache.service';
...
const AdminEstadosCivis: React.FC = () => {
  const { user, loading } = useCurrentUser();
  const [rows, setRows] = useState<EstadoCivilRow[]>([]);
  ...
  const load = async () => {
    ...
    setLoadingData(true);
    try {
      const data = await fetchCachedEstadosCivis(user.companyId);
      setRows((data ?? []).map((r) => ({
        id: String(r.id ?? ''),
        name: String(r.name || ''),
      ...
      setMessage({ type: 'error', text: 'Erro ao carregar estados civis.' });
    } finally {
      setLoadingData(false);
    }
  };
  ...
  const openEdit = (row: EstadoCivilRow) => {
    setEditingId(row.id);
    setName(row.name);
    setModalOpen(true);
    setMessage(null);
  ...
  const handleSave = async (e?: React.MouseEvent) => {
    ...
    if (!trimmed) {
      setModalError('Informe a descrição (nome do estado civil).');
      return;
    }
    setSaving(true);
    setModalError(null);
    ...
    try {
      if (editingId) {
        await db.update('estados_civis', editingId, { name: trimmed });
        setMessage({ type: 'success', text: 'Estado civil atualizado com sucesso.' });
      } else {
        await db.insert('estados_civis', {
        ...
        setMessage({ type: 'success', text: 'Estado civil cadastrado com sucesso.' });
      }
      setModalOpen(false);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Excluir este estado civil? Funcionários vinculados podem ficar sem estado civil.')) return;
    try {
      await db.delete('estados_civis', id);
      setMessage({ type: 'success', text: 'Estado civil excluído.' });
      if (user?.companyId) invalidateStaticCatalogCaches(user.companyId);
      load();
    } catch (e: any) {
    ...
    <RoleGuard user={user} allowedRoles={['admin', 'hr']}>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <PageHeader
            title="Estados Civis"
            subtitle="Cadastro de estados civis. Vincule em Funcionários > Dados Adicionais."
            icon={<Heart size={24} />}
          />
          <button
            type="button"
          ...
          {loadingData ? (
            ...
            <>
              ...
              {!loadingData && rows.length === 0 && (
                <p className="p-8 text-center text-slate-500 dark:text-slate-400">Nenhum estado civil cadastrado. Clique em &quot;Incluir&quot; para começar.</p>
              )}
            </>
          )}
        </div>
        ...
        {modalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" role="dialog" aria-modal="true">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 w-full max-w-md p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">{editingId ? 'Editar estado civil' : 'Novo estado civil'}</h3>
              {modalError && (
...
export default AdminEstadosCivis;
