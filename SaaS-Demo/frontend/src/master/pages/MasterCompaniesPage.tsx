...
import type { CompanyLicense } from '../services/masterLicenseManagerService';
...
export function MasterCompaniesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const licenses = await MasterLicenseManagerService.list().catch(() => [] as CompanyLicense[]);
      const licenseByTenant = new Map<string, CompanyLicense>(licenses.map((l) => [l.tenantId, l] as const));
