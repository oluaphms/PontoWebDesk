import { FRONTEND_SERVICE_NAME } from './FrontendServiceConfig.js';
import { netExecutor, parseScQueryState, type ScExecutor } from '../scExec.js';

export type FrontendServiceRunState = 'RUNNING' | 'STOPPED' | 'NOT_INSTALLED' | 'UNKNOWN';

export interface FrontendServiceStatus {
  installed: boolean;
  state: FrontendServiceRunState;
  raw?: string;
}

export class FrontendServiceController {
  constructor(private readonly sc: ScExecutor) {}

  query(): FrontendServiceStatus {
    ...
    let state: FrontendServiceRunState = 'UNKNOWN';
  ...
  start(): { ok: boolean; message: string } {
    ...
    const r = netExecutor('start', FRONTEND_SERVICE_NAME);
    if (r.exitCode !== 0) {
      const scStart = this.sc(['start', FRONTEND_SERVICE_NAME]);
  ...
  stop(): { ok: boolean; message: string } {
    ...
    const r = netExecutor('stop', FRONTEND_SERVICE_NAME);
    if (r.exitCode !== 0) {
      const scStop = this.sc(['stop', FRONTEND_SERVICE_NAME]);
