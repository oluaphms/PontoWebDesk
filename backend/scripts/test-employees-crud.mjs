...
console.log('[OK] login', companyId);
...
console.log('[LIST]', found ? 'campos OK' : 'não encontrado', found);
...
console.log('[PATCH]', updated.status, updated.json?.employee?.cargo);

const removed = await req('DELETE', `/employees/${id}`, null, token);
console.log('[DELETE]', removed.status, removed.json);
console.log('[DONE] CRUD colaboradores');
