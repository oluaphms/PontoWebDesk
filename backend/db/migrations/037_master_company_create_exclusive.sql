...
-- Bloqueia INSERT autenticado em public.companies e desativa RPC legada.
-- Idempotente. Empresas existentes permanecem intactas.
-- Control plane Master (queryMaster / service role) continua podendo INSERT.

BEGIN;

-- 1) Política: usuários autenticados do SaaS NÃO criam companies.
...
-- 2) RPC legada create_tenant_onboarding → sempre nega (compatível se a função existir).
DO $$
BEGIN
  IF EXISTS (
