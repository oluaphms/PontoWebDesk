...
/** GET /api/master/licenses — cloud + locais (Fase 25 foca locais). */
export async function getMasterLicensesController(_req: Request, res: Response): Promise<void> {
  try {
    const cloud = await MasterPlatformService.getDashboard().licenses.list();
    const localRows = await MasterPlatformService.getLocalLicense().list();
    const localLicenses = await Promise.all(localRows.map((r) => toView(r)));
    res.json({
      ok: true,
      licenses: cloud,
      localLicenses,
      ...
      note: 'Controle de licenças locais via LocalLicenseManager (offline)',
    });
  } catch (error) {
