...
export class MasterDashboardService {
  ...
  private buildExecutiveFromLists(input: {
    ...
    const recentFromInvoices = paidInvoices
      .slice()
      .sort((a, b) => Date.parse(b.paidAt || b.issuedAt) - Date.parse(a.paidAt || a.issuedAt))
      .slice(0, 8)
      .map((inv) => ({
        id: inv.id,
        label: `Fatura ${inv.id.slice(0, 8)}`,
        amountCents: inv.amountCents,
        status: inv.status,
        method: 'invoice',
        at: inv.paidAt || inv.issuedAt,
      }));
    const recentPayments = [...recentFromPayments, ...recentFromInvoices]
