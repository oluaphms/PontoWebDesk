...
if not defined DEMO_PG_PORT set "DEMO_PG_PORT=5432"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $r=Invoke-WebRequest -Uri 'http://localhost:3000/api/health' -UseBasicParsing -TimeoutSec 4; Write-Output ($r.StatusCode.ToString() + ' ' + $r.Content); exit 0 } catch { Write-Output $_.Exception.Message; exit 1 }" > "%TEMP%\saas_be_health.txt" 2>&1
if errorlevel 1 (
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $r=Invoke-WebRequest -Uri 'http://localhost:3010' -UseBasicParsing -TimeoutSec 4; Write-Output $r.StatusCode; exit 0 } catch { Write-Output $_.Exception.Message; exit 1 }" > "%TEMP%\saas_fe_health.txt" 2>&1
if errorlevel 1 (
