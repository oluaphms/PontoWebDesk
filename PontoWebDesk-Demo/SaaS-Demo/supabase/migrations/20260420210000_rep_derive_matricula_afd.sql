-- Deriva crachá (ex. 705412) do campo AFD de 11 dígitos com zeros à esquerda (ex. 00000705412),
-- igual ao TypeScript matriculaFromAfdPisField — para novas ingestões e para filas antigas em rep_promote.
...
'Control iD/AFD: campo 11 posições pode ser crachá com zeros à esquerda; retorna só o número (ex. 705412).';
  ...
  v_user_id := (
    ...
    WHERE u.company_id = p_company_id
      AND (
        ...
        OR (v_matricula_norm IS NOT NULL AND (
          trim(COALESCE(u.numero_folha, '')) = v_matricula_norm
          OR trim(COALESCE(u.numero_identificador, '')) = v_matricula_norm
        ))
    ...
    WHERE u.company_id = l.company_id
      AND (
        ...
        OR (
          ...
          AND (
            trim(COALESCE(u.numero_folha, '')) = NULLIF(trim(l.matricula), '')
            OR trim(COALESCE(u.numero_identificador, '')) = NULLIF(trim(l.matricula), '')
          )
        )
        OR (
          public.rep_derive_matricula_from_afd_11(COALESCE(l.pis, l.cpf, '')) IS NOT NULL
          AND (
            trim(COALESCE(u.numero_folha, '')) = public.rep_derive_matricula_from_afd_11(COALESCE(l.pis, l.cpf, ''))
            OR trim(COALESCE(u.numero_identificador, '')) = public.rep_derive_matricula_from_afd_11(COALESCE(l.pis, l.cpf, ''))
          )
        )
        OR (
...
COMMENT ON COLUMN public.users.numero_identificador IS
'Nº crachá/digital — único no sistema; também usado para casar matrícula enviada pelo Control iD (campo matricula em rep_punch_logs).';
