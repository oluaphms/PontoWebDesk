-- 1) normalize_time_record_punch_type: códigos de uma letra (E/S/P) e variantes comuns do REP.
--    Antes caíam no ELSE ('e','s','p') e eram excluídos de WHERE typ IN ('entrada','saida','pausa'),
--    quebrando a ordem civil e mantendo E→E na validação.
-- 2) Dupla entrada em batidas REP/relógio: limiar 1s em vez de 45s (variável v_dup_thresh no trigger).
...
COMMENT ON FUNCTION public.normalize_time_record_punch_type(text) IS
  'Normaliza tipo para sequência E/P/S; inclui E/S/P e B (REP).';
...
COMMENT ON FUNCTION public.time_records_enforce_punch_sequence() IS
  'BEFORE INSERT: normalização E/S/P; dupla entrada REP >1s (outros >45s); reconciliação + Portaria 671.';
