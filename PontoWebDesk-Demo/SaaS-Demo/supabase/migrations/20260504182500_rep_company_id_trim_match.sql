-- Match REP: ignorar espaços em `company_id` (users / rep_punch_logs / parâmetro) — evita fila sem match por whitespace.
...
AS $$
  SELECT u.id
  FROM public.users u
  WHERE btrim(u.company_id::text) = btrim(COALESCE(p_company_id, ''))
    AND (
...
COMMENT ON FUNCTION public.rep_resolve_user_id_for_rep_match(text, text, text, text) IS
  'REP: user.id com prioridade PIS; company_id comparado com btrim nos dois lados.';
...
BEGIN
  ...
  v_user_uuid := public.rep_resolve_user_id_for_rep_match(
    v_cid, v_pis_norm, v_cpf_norm, v_matricula_norm
  );
...
COMMENT ON FUNCTION public.rep_promote_pending_rep_punch_logs(text, uuid, timestamptz, timestamptz, uuid) IS
  'Promove rep_punch_logs; company_id com btrim; blob + crachá; timeout 600s.';
