-- Consolidação REP: evitar cancelamento por statement_timeout em tenants com muita fila;
-- índice parcial para varrer só pendências por empresa/relógio.

CREATE INDEX IF NOT EXISTS idx_rep_punch_logs_pending_promote
  ON public.rep_punch_logs (company_id, rep_device_id, data_hora)
...
COMMENT ON FUNCTION public.rep_promote_pending_rep_punch_logs(text, uuid, timestamptz, timestamptz, uuid) IS
  'Promove rep_punch_logs; timeout local alargado; ignora linhas marked ignored; índice pendente.';
