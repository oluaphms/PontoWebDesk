-- Extrair linha AFD compacta de raw_data sem confundir envelope JSON (raw object) com linha numérica.
-- Antes: raw_data->>'raw' com raw = object devolvia texto JSON; o blob identificador nunca era lido e o
-- fallback numero_identificador vs blob não corria.

CREATE OR REPLACE FUNCTION public.rep_compact_afd_line_from_punch_raw(p_raw jsonb)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
...
BEGIN
  ...
  cur := p_raw;
  FOR i IN 1..5 LOOP
    IF jsonb_typeof(cur) <> 'object' OR NOT (cur ? 'raw') THEN
      EXIT;
    END IF;
    j := cur->'raw';
...
COMMENT ON FUNCTION public.rep_compact_afd_line_from_punch_raw(jsonb) IS
  'Linha AFD compacta (tipo 3/7): percorre raw object → raw string como em collectRawDataLayers (TS).';

GRANT EXECUTE ON FUNCTION public.rep_compact_afd_line_from_punch_raw(jsonb) TO authenticated, service_role;
    ...
    IF v_user_uuid IS NULL
      ...
      v_user_uuid := (
        ...
        WHERE u.company_id::text = p_company_id
          AND length(regexp_replace(COALESCE(u.numero_identificador, ''), '\D', '', 'g')) >= 8
          AND v_id_blob_d LIKE concat(
            regexp_replace(COALESCE(u.numero_identificador, ''), '\D', '', 'g'),
            '%'
          )
        ORDER BY length(regexp_replace(COALESCE(u.numero_identificador, ''), '\D', '', 'g')) DESC, u.id
        LIMIT 1
      );
    END IF;
...
COMMENT ON FUNCTION public.rep_promote_pending_rep_punch_logs(text, uuid, timestamptz, timestamptz, uuid) IS
  'Promove rep_punch_logs; linha AFD via rep_compact_afd_line_from_punch_raw; fallback prefixo numero_identificador; timeout 600s.';
