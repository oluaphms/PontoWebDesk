-- Match de crachá/matrícula: além de igualdade literal (trim), compara o valor numérico dos
-- dígitos (705412 = 0705412 = 00000705412). Cobre cadastro sem zeros à esquerda e AFD preenchido.

CREATE OR REPLACE FUNCTION public.rep_matricula_matches_user_fields(
  p_matricula text,
  p_numero_folha text,
  p_numero_identificador text
)
RETURNS boolean
LANGUAGE plpgsql
IMMUTABLE
...
DECLARE
  ...
  id text;
BEGIN
  IF p_matricula IS NULL OR length(trim(COALESCE(p_matricula, ''))) = 0 THEN
    RETURN FALSE;
  END IF;

  vm := trim(p_matricula);
  nf := trim(COALESCE(p_numero_folha, ''));
  ni := trim(COALESCE(p_numero_identificador, ''));

  IF nf = vm OR ni = vm THEN
    RETURN TRUE;
  END IF;

  vd := NULLIF(regexp_replace(vm, '\D', '', 'g'), '');
  IF vd IS NULL OR vd !~ '^\d+$' THEN
    RETURN FALSE;
  END IF;

  fd := NULLIF(regexp_replace(nf, '\D', '', 'g'), '');
  id := NULLIF(regexp_replace(ni, '\D', '', 'g'), '');

  IF fd IS NOT NULL AND fd ~ '^\d+$' AND fd::numeric = vd::numeric THEN
    RETURN TRUE;
  END IF;
  IF id IS NOT NULL AND id ~ '^\d+$' AND id::numeric = vd::numeric THEN
    RETURN TRUE;
  END IF;
...
COMMENT ON FUNCTION public.rep_matricula_matches_user_fields(text, text, text) IS
'Casamento matrícula/crachá REP com numero_folha e numero_identificador: literal ou mesmo valor numérico dos dígitos.';
