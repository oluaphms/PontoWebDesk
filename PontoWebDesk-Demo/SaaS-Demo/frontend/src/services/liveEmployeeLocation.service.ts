...
export async function fetchLiveLocationsForCompany(
  companyId: string,
  clientOverride?: unknown | null,
): Promise<LiveEmployeeLocationRow[]> {
  const client = (clientOverride ?? getSupabaseClient()) as unknown as LiveLocationReadClient | null;
...
export async function upsertLiveEmployeeLocation(
  ...
  const client = (clientOverride ?? getSupabaseClient()) as unknown as LiveLocationWriteClient | null;
...
export async function runLiveLocationCleanup(clientOverride?: unknown | null): Promise<number> {
  const client = (clientOverride ?? getSupabaseClient()) as
