...
export async function uploadPunchPhotoWithRetry(
  ...
  const uploadFailure = apiUpload as Extract<typeof apiUpload, { ok: false }>;
  if (!isTransientUploadError(new Error(uploadFailure.error))) {
    return { publicUrl: null, error: uploadFailure.error, transientFailure: false };
  }
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (!apiRetry.ok) {
        const retryFailure = apiRetry as Extract<typeof apiRetry, { ok: false }>;
        throw new Error(retryFailure.error);
      }
