...
export async function runOperationalStateSelfHeal(
  ...
  observabilityConsole.info('[SELF HEAL START]', { company_id: companyId, correlation_id: correlationId });
  try {
    ...
    if (!hadIssues) {
      observabilityConsole.info('[SELF HEAL SUCCESS]', { company_id: companyId, repaired: 0, note: 'no_drift' });
      return { ok: true, repaired: 0, hadIssues: false };
    }
    observabilityConsole.info('[SELF HEAL SUCCESS]', {
    ...
    observabilityConsole.warn('[SELF HEAL FAILED]', { company_id: companyId, error: String(e) });
